...
export default function SortableComponent({ component }: SortableComponentProps) {
    ...
    const renderContent = () => {
        switch (component.type) {
            ...
            case 'DataTable':
                return (
                    <div className="w-full h-48 border-2 border-dashed border-zinc-700 rounded bg-zinc-800/50 flex items-center justify-center flex-col">
                        <span className="font-mono text-zinc-400">{'<DataTable />'}</span>
                        <span className="text-xs text-zinc-500 mt-1">Bound to: {component.props.entityName || 'None'}</span>
                    </div>
                );
            case 'FormModule':
                return (
                    <div className="w-full h-48 border-2 border-dashed border-emerald-700/50 rounded bg-emerald-900/10 flex items-center justify-center flex-col">
                        <span className="font-mono text-emerald-500/70">{'<FormModule />'}</span>
                        <span className="text-xs text-emerald-500/50 mt-1">Bound to: {component.props.entityName || 'None'}</span>
                    </div>
                );
            default:
    ...
    return (
        <div
            ...
            className={`relative p-4 rounded-md border-2 bg-zinc-900 shadow-sm cursor-grab group transition-colors ${isDragging ? 'opacity-50 z-50 shadow-xl' : 'opacity-100'
                } ${isSelected ? 'border-indigo-500' : 'border-zinc-800 hover:border-zinc-700'
                }`}
        >
