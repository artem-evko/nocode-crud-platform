...
export default function ${entity.name()}List() {
  const [data, setData] = useState<${entity.name()}[]>([]);
  ...
  const fetchData = async () => {
    try {
      const response = await api.get('/${entity.table()}');
    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-zinc-500 mt-1">Manage your ${entity.name()?lower_case} records</p>
        </div>
        <Link 
          to="/${entity.name()?lower_case}/new" 
                ...
                <tr>
                  <td colSpan={100} className="px-6 py-12 text-center text-zinc-500">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-sm">Get started by creating a new ${entity.name()}</p>
                    </div>
                  </td>
                </tr>
              ) : (
